...
import * as fontmachine from 'fontmachine';
...
export class TaskFSHandler extends EventEmitter {
    ...
    getFileList(inDir: fs.PathLike) {
        ...
        for (let i in allFiles) {
            let matchingFiles: string[] = allFiles[i].match(/(ttf|woff|woff2|eot|otf|svg)$/);
    ...
    getFontMetadata(source: string, fileList: string[]) {
        return Promise.all(fileList.map(function (fileName) { // note: promise is outside.. somehow.. yea..
    ...
    cleanFontMetadataArray(fontMetadataArray: any[]) { // fontmachine[ [{}, 'str'], [undefined, 'str'] ]
    ...
    groupFonts(addFontWeightProp: any[]) {
        let groupFontsArray = [];
        for (let i = 0; i < addFontWeightProp.length; i++) { //26 times
            for (let idx = 0; (idx-1) < groupFontsArray.length; idx++) {
                if (groupFontsArray[idx] === undefined) {
                    groupFontsArray.push([addFontWeightProp[i]]); break;       
                    ...
                    groupFontsArray[idx].push(addFontWeightProp[i]); break;
                } 
            }
        }
    ...
    getNoMetadataList(fileList: string[], fontMetadataArray: {}[]) {
        let noMetaFileNames: string[] = [];
    ...
    createFontFaceJson(groupFonts: any[], getNoMetadataList: any[]) {
        let fontFaceJson = {
            autofont: [],
            fixme: []
        };
    ...
    checkIfDataExist(inDir) { // output is tied with @Emitter
        let allFiles: string[];
        let autofontFSMatch: any;
        let autofontData: string;
    ...
    writeScssFile(inDir, MT_createFontFaceString) { // @Emitter
    ...
    tmStart(inDir) { // @Emitter
        ...
        .then((MTV_fontMetadataArray) => { // => [ [{}, 'filetype', 'filename'], [undefined, 'filetype', 'filename'] ] 
            // Part 1
            ...
            if (MT_checkIfDataExist === false) this.emit('addCount');
