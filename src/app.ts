...
/* 
	================================== Readme ======================================
	Definitions:
	Task - refer to single module object/class that use the taskmaster api. 
		They will peform their assigned job by calling tmStart('arguments (if any)')
		e.g let myTask = new importedTaskModule; myTask.tmStart();
	
	Readme:
	- FS watch calls tasks that can cause an infinite loop (as they implement fs changes too).
		In order to prevent this the tasks emit an event when they execute an fs.write. 
		A queue count will determine when it is safe for fs.watch to call the tasks again.
*/
...
//--- Project Modules ---
import { TaskFSHandler } from './taskFSHandler';
import { TaskScssSaveHandler } from './taskScssSaveHandler';
...
if (source && watch) {
	...
	fs.watch(source, 'utf-8', function(eventType, filename) {
		...
		if (count != 0) {
			...
			if (count === 0 && filename === 'autofont.scss') dirCount = fs.readdirSync(source).length;
		}
	});
} else if (source) {
	console.log(welcomeMessage);
